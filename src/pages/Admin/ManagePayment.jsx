...
const ManagePayment = () => {
  const recentPayments = [
    { id: 'PAY001', method: 'Thẻ tín dụng', name: 'Nguyễn Văn A', amount: '500.000 VNĐ', date: '2025-04-15' },
    { id: 'PAY002', method: 'Paypal', name: 'Lê Thị B', amount: '1.200.000 VNĐ', date: '2025-04-14' },
  ];

  const [packages, setPackages] = useState([
    { id: 'PG001', name: 'Lifetime', description: 'Full bộ đề TOEIC các năm', cost: '500.000 VNĐ', duration: 'Trọn đời' },
  ]);
  ...
  const handleAddOrUpdatePackage = (formData) => {
    if (currentPackage) {
      ...
      const newPackage = { ...formData, id: `PG${(packages.length + 1).toString().padStart(3, '0')}` };
  ...
  const currentPayments = recentPayments.slice(startIndexPayments, endIndexPayments);
    ...
    <main className="max-w-6xl w-full mx-auto space-y-6 p-4">
      ...
      <section className="mx-auto max-w-4xl flex flex-col space-y-3">
        <div className="flex border-2 border-gray-200 rounded-xl space-x-5 shadow-md p-5 items-center justify-around">
          <div className="flex gap-2 items-center justify-center text-md text-gray-600 font-semibold">
            ...
            <span className="font-bold text-[#2C99E2]">8</span>
          </div>
          <div className="flex gap-2 items-center text-md justify-center text-gray-600 font-semibold">
            <span>Doanh thu:</span>
            <span className="font-bold text-[#2C99E2]">3.000.000 VNĐ</span>
          </div>
        </div>
      </section>
      ...
      <section className="flex flex-col py-5 px-8 gap-5 border-2 border-gray-200 rounded-xl shadow-md">
        ...
        <div className="flex flex-row gap-4 w-full px-6">
          <SearchBar text="Tìm kiếm giao dịch theo id hoặc tên người dùng" focusBorderColor="focus:ring-gray-400" />
          <div className="flex gap-2 items-center">
            <label htmlFor="filterType" className="text-sm font-medium">Lọc theo:</label>
            <select
              id="filterType"
              name="filterType"
              className="border-2 border-gray-300 text-gray-600 font-semibold rounded-md p-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-400"
            >
          ...
          <tbody>
            {currentPayments.map((payment) => (
              <tr key={payment.id} className="hover:bg-gray-100">
                <td className="px-4 py-4 text-gray-600 font-semibold">{payment.id}</td>
                <td className="px-4 py-4 font-bold text-[#2C99E2]">{payment.method}</td>
                <td className="px-4 py-4 text-gray-600 font-semibold">{payment.name}</td>
                <td className="px-4 py-4 font-bold text-[#2C99E2]">{payment.amount}</td>
                <td className="px-4 py-4 text-gray-600 font-semibold">{payment.date}</td>
                <td className="px-4 py-4 text-center">
                  ...
                  />
                  <DetailPayment show={showDetailPayment} onClose={() => setShowDetailPayment(false)} />
                </td>
              </tr>
            ))}
          </tbody>
        ...
        {totalPayments > 0 && (
          <div className="flex justify-between items-center p-4">
            <span className="text-sm text-gray-600 font-semibold">
              Hiển thị từ {startIndexPayments + 1} đến {Math.min(endIndexPayments, totalPayments)}{" "}
              trong số {totalPayments} giao dịch
            </span>
            <Pagination
      ...
      <section className="flex flex-col py-5 px-8 gap-5 border-2 border-gray-200 rounded-xl shadow-md">
        ...
        <div className="flex flex-row gap-4 w-full px-6 py-3">
          <SearchBar text="Tìm kiếm gói học theo tên" focusBorderColor="focus:ring-gray-400" />
          <Button
            text="Thêm mới"
            variant="primary"
            size="sm"
          ...
          <tbody>
            {currentPackages.map((pkg) => (
              <tr key={pkg.id} className="hover:bg-gray-100">
                <td className="px-4 py-4 text-gray-600 font-semibold">{pkg.id}</td>
                <td className="px-4 py-4 font-bold text-[#2C99E2]">{pkg.name}</td>
                <td className="px-4 py-4 text-gray-600 font-semibold">{pkg.description}</td>
                <td className="px-4 py-4 font-bold text-[#2C99E2]">{pkg.cost}</td>
                <td className="px-4 py-4 text-gray-600 font-semibold">{pkg.duration}</td>
        ...
        {totalPackages > 0 && (
          <div className="flex justify-between items-center p-4">
            <span className="text-sm text-gray-600 font-semibold">
              Hiển thị từ {startIndexPackages + 1} đến {Math.min(endIndexPackages, totalPackages)}{" "}
              trong số {totalPackages} gói học
            </span>
            <Pagination
