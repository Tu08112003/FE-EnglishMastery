...
const Dashboard = () => {
  const recentPayments = [
    { id: 'PAY001', name: 'Nguyễn Văn A', amount: '1.000.000 VNĐ', date: '2025-04-01' },
    { id: 'PAY002', name: 'Trần Thị B', amount: '500.000 VNĐ', date: '2025-04-05' },
    { id: 'PAY003', name: 'Lê Văn C', amount: '750.000 VNĐ', date: '2025-04-10' },
  ];

  const recentExams = [
    { id: 'EXAM001', name: 'Nguyễn Văn A', score: "850/990", date: '2025-04-02' },
    { id: 'EXAM002', name: 'Trần Thị B', score: "620/990", date: '2025-04-06' },
    { id: 'EXAM003', name: 'Lê Văn C', score: "780/990", date: '2025-04-11' },
  ];
  ...
  const currentExams = recentExams.slice(startIndexExams, endIndexExams);
    ...
    <main className="max-w-6xl w-full mx-auto space-y-6 p-4">
      <section className="flex flex-col space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 justify-items-center items-center">
          <StatisticsCard
            icon={<FontAwesomeIcon icon="fa-solid fa-file" size="4x" style={{ color: "#2C99E2" }} />}
            value="5"
            description="Đề thi đã làm"
          />
          <StatisticsCard
            icon={<FontAwesomeIcon icon="fa-solid fa-dollar-sign" size="4x" style={{ color: "#2C99E2" }} />}
            value="5.000.000 VNĐ"
            description="Doanh thu"
          />
          <StatisticsCard
            icon={<FontAwesomeIcon icon="fa-solid fa-users" size="4x" style={{ color: "#2C99E2" }} />}
            value="500"
      ...
      <section className="flex flex-col space-y-4">
        ...
        <div className="space-y-6 border-2 border-gray-200 rounded-lg px-10 py-8 shadow-xl">
                ...
                <tbody>
                  {currentPayments.map((payment) => (
                    <tr key={payment.id} className="hover:bg-gray-100">
                      <td className="px-4 py-2 text-gray-600 font-semibold">{payment.id}</td>
                      <td className="px-4 py-2 text-gray-600 font-semibold">{payment.name}</td>
                      <td className="px-4 py-2 font-bold text-[#2C99E2]">{payment.amount}</td>
                      <td className="px-4 py-2 text-gray-600 font-semibold">{payment.date}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="flex flex-col gap-1">
            <h2 className="text-lg font-semibold mb-2">Lịch sử làm đề gần đây</h2>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[600px] text-center border-2 border-gray-300 rounded-2xl overflow-hidden border-separate border-spacing-0">
                <thead className="bg-gray-200">
                  <tr className="text-black font-bold">
                ...
                <tbody>
                  {currentExams.map((exam) => (
                    <tr key={exam.id} className="hover:bg-gray-100">
                      <td className="px-4 py-2 font-semibold text-gray-600">{exam.id}</td>
                      <td className="px-4 py-2 font-semibold text-gray-600">{exam.name}</td>
                      <td className="px-4 py-2 font-bold text-[#2C99E2]">{exam.score}</td>
                      <td className="px-4 py-2 font-semibold text-gray-600">{exam.date}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
